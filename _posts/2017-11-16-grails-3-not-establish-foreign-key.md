...
//            value.createForeignKey()
    ...
    protected void bindSimpleValue(PersistentProperty grailsProp,
        ...
        if (mappedForm.isDerived() && !(grailsProp instanceof TenantId)) {
            ...
            List<?> columnDefinitions = hasConfig ? propertyConfig.getColumns()
                    : Arrays.asList([null] as Object[])
            if (columnDefinitions.isEmpty()) {
                columnDefinitions = Arrays.asList([null] as Object[])
            }
    ...
    protected void bindColumn(PersistentProperty property, PersistentProperty parentProperty,
        ...
        if (!owner.isRoot()) {
            ...
            if (mapping == null || mapping.getTablePerHierarchy()) {
                if (LOG.isDebugEnabled())
                    LOG.debug("[MyGrailsDomainBinder] Sub class property [" + property.getName() + "] for column name ["+column.getName()+"] set to nullable")
                column.setNullable(true)
            } else {
                column.setNullable(property.isNullable())
            }
        ...
        if (LOG.isDebugEnabled())
            LOG.debug("[MyGrailsDomainBinder] bound property [" + property.getName() + "] to column name ["+column.getName()+"] in table ["+table.getName()+"]")
    }
    ...
    protected void bindNumericColumnConstraints(Column column, PersistentProperty property, ColumnConfig cc) {
        ...
        if( cc != null && cc.getPrecision() > -1) {
            ...
            if (minConstraintValueLength > 0 && maxConstraintValueLength > 0) {
                ...
                precision = DefaultGroovyMethods.max([precision, minConstraintValueLength, maxConstraintValueLength] as Integer[])
            }
    ...
    class GrailsCollectionSecondPass implements SecondPass {
        ...
        protected String sessionFactoryBeanName
         ...
         GrailsCollectionSecondPass(ToMany property, InFlightMetadataCollector mappings,
         ...
         void doSecondPass(Map<?, ?> persistentClasses, Map<?, ?> inheritedMetas) throws MappingException {
    ...
    class ListSecondPass extends GrailsCollectionSecondPass {
        private static final long serialVersionUID = -3024674993774205193L
         ...
         ListSecondPass(ToMany property, InFlightMetadataCollector mappings,
                              Collection coll, String sessionFactoryBeanName) {
    ...
    class MapSecondPass extends GrailsCollectionSecondPass {
        private static final long serialVersionUID = -3244991685626409031L
         ...
         MapSecondPass(ToMany property, InFlightMetadataCollector mappings,
                             Collection coll, String sessionFactoryBeanName) {
            super(property, mappings, coll, sessionFactoryBeanName)
        }
    ...
    static abstract class CollectionType {
        ...
        protected final MyGrailsDomainBinder binder
        ...
        protected static final Map<Class<?>, CollectionType> INSTANCES = new HashMap<Class<?>, CollectionType>()
         ...
         abstract Collection create(ToMany property, PersistentClass owner,
                                          String path, InFlightMetadataCollector mappings, String sessionFactoryBeanName) throws MappingException

        protected CollectionType(Class<?> clazz, MyGrailsDomainBinder binder) {
            this.clazz = clazz
            this.binder = binder
        ...
        protected void createInstances() {
            ...
            SET = new CollectionType(Set.class, binder) {
                @Override
                 Collection create(ToMany property, PersistentClass owner,
                                         String path, InFlightMetadataCollector mappings, String sessionFactoryBeanName) throws MappingException {
         ...
         CollectionType collectionTypeForClass(Class<?> clazz) {
            createInstances()
